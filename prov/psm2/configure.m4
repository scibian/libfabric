	       ...
	       [AS_IF([test x$have_psm2_src = x0],
			...
			AS_IF([test x$psm2_happy = x1],
			      AS_IF([test -f $with_psm2_src/libpsm2.spec.in],
					...
					AS_IF([grep -q psm2_mq_fp_msg $with_psm2_src/psm2_mq.h],
					   [
						$as_echo "$as_me: psm2_mq_fp_msg exists in the psm2 source."
					   ],
					   [
						$as_echo "$as_me: PSM2 source under <$with_psm2_src> does not provide psm2_mq_fp_msg."
						have_psm2_mq_fp_msg=0
					   ])
				   ],
				   [
